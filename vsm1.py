...
class AnchorFirstSemanticSubgraphMatcher:
    """
    Exact semantic subgraph matching with:
    1) injective mapping
    2) query edge preservation only (non-induced)
    3) node semantic similarity >= tau
    4) anchor-first strategy:
         - only the anchor query node gets global exact candidates
         - other query nodes get local candidates during expansion
    5) minimum-candidate-first after the anchor is matched

    Anchor rule:
      choose the query node whose summed similarity to all other query nodes
      is minimal (more semantically distinctive inside the query).
    """

    def __init__(self, data_graph: Data, query_graph: Data, tau: float):
        ...
        # only build global candidates for the anchor
    ...
    def _choose_anchor_query_node_by_uniqueness(self) -> int:
        """
        Choose the query node whose summed similarity to all other query nodes is minimal.
        Tie-break:
          1) larger query degree first
          2) smaller node id
        """
        sim_sums = []
        ...
        for q_u in range(self.num_q):
            print(
                f"  q{q_u}: sim_sum={sim_sums[q_u]:.6f}, deg={self.Q_deg[q_u]}"
            )
    ...
    def _build_initial_candidates_for_anchor_only(self) -> Dict[int, Set[int]]:
        """
        Only the anchor gets global exact candidates.
        All other query vertices start with empty sets and will be expanded locally.
        """
        candidates: Dict[int, Set[int]] = {q_u: set() for q_u in range(self.num_q)}

        q_anchor = self.anchor_q
        q_vec = self.Q_x[q_anchor]
        sims = self.G_x @ q_vec

        cur = set()
        for g_v in range(self.num_g):
            if self.G_deg[g_v] < self.Q_deg[q_anchor]:
                continue
            if float(sims[g_v]) >= self.tau:
                cur.add(g_v)

        candidates[q_anchor] = cur
        return candidates
    ...
    def _generate_local_candidates(
        ...
        """
        Generate candidates for q_u locally from already matched neighbors only.

        If q_u has matched query neighbors, its candidates must lie in the
        intersection of the neighborhoods of their mapped data nodes.

        Then apply:
          - injective constraint
          - degree filter
          - exact semantic threshold
          - already matched edge consistency
        """
        matched_neighbors = [q_n for q_n in self.Q_adj[q_u] if q_n in mapping]

        # In this anchor-first framework, local generation should only happen
        # for vertices that are adjacent to at least one already matched vertex.
        if len(matched_neighbors) == 0:
            return set()

        # Start from neighborhood intersection of mapped neighbors
        ...
        for g_v in candidate_region:
            ...
            # preserve edges to all already matched neighbors of q_u
            feasible = True
            for q_m, g_m in mapping.items():
                if q_m in self.Q_adj[q_u]:
                    if g_m not in self.G_adj[g_v]:
                        feasible = False
                        break
    ...
    def _select_next_query_node(
        ...
        """
        Selection policy:
        1) the anchor must be selected first
        2) after that, prefer unmatched nodes having at least one matched neighbor
        3) among them, minimum-candidate-first
        Tie-break:
          - more matched neighbors first
          - larger query degree first
          - smaller node id
        """
        ...
        for q_u in unmatched_q_nodes:
            ...
            if matched_nbr_cnt > 0:
                expandable.append((q_u, matched_nbr_cnt))

        if expandable:
            return min(
                [q_u for q_u, _ in expandable],
                key=lambda q_u: (
        ...
        # Fallback, though for a connected query this usually should not happen
        return min(
            unmatched_q_nodes,
            key=lambda q_u: (-self.Q_deg[q_u], q_u)
        )
    ...
    def _forward_check(
        ...
        """
        After assigning q_u -> g_v:
          - already materialized candidate sets are shrunk
          - not-yet-materialized candidate sets can remain empty
          - if an unmatched node is adjacent to the newly matched part and still
            has empty candidates, we generate its local candidates on demand
        """
        ...
        for q_w in unmatched_q_nodes:
            ...
            # If q_w has not yet been materialized, and now has matched neighbors,
            # generate its local candidates on demand.
            ...
            # if q_w is adjacent to q_u, q_w must lie in neighbors of g_v
            if q_w in self.Q_adj[q_u]:
                cur = cur & self.G_adj[g_v]

            # also preserve edges to all already matched neighbors
            ...
            # semantic threshold recheck is not needed here because
            # local generation already enforced it, and anchor candidates
            # were built by exact semantic filtering.
    ...
    def find_all_matches(self, max_matches: int | None = None):
        # anchor must have non-empty global candidates
        ...
        def backtrack(
            ...
            # If a non-anchor node is selected but still has no candidates,
            # try to generate its local candidates now.
...
def main():
    ...
    matcher = AnchorFirstSemanticSubgraphMatcher(
        data_graph=data_graph,
